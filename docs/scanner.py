    ...
    try:
        ...
        for sym in symbols:
            ...
            if result and (result["raw_symbol"], result["direction"], result["timeframe"]) not in open_set:
                ...
                logger.info(f"[1h] {result['symbol']} {result['direction']}")

        if ENABLE_15M:
            logger.info("[15m] 스캔 시작...")
            for sym in symbols:
                result = scan_symbol(sym, "15m")
                if result and (result["raw_symbol"], result["direction"], result["timeframe"]) not in open_set:
                    new_signals.append(result)
                    open_set.add((result["raw_symbol"], result["direction"], result["timeframe"]))
                    logger.info(f"[15m] {result['symbol']} {result['direction']}")

        logger.info(f"새 시그널 {len(new_signals)}건")
